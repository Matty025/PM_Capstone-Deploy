...
function PredictiveMaintenance() {
  ...
  const fetchRows = async (motorcycle_id, mins, { silent = false } = {}) => {
    ...
    try {
        const { data } = await axios.post(`${process.env.REACT_APP_PYTHON_SERVER_URL}/recent-data`, {
        motorcycle_id,
        minutes: mins,
      });
      if (data.status === "ok") {
...
useEffect(() => {
  ...
  try {
    if (uploadedFile && useUploadedFile) {
      ...
      const { data } = await axios.post(`${process.env.REACT_APP_PYTHON_SERVER_URL}/predict-from-csv`, formData, {
  ...
  toast.info("📡 Using latest InfluxDB sensor data for analysis...");
    ...
    const { data } = await axios.post(`${process.env.REACT_APP_PYTHON_SERVER_URL}/predict`, {
    motorcycle_id: id,
    brand,
    model,
  });
...
const saveRecentDataAsCSV = () => {
  ...
  try {
      const res = await axios.post(`${process.env.REACT_APP_PYTHON_SERVER_URL}/train_model`, {
      motorcycle_id,
      brand,
    });
